...
async function renderOrders(container, collapseState, onOrderSelect) {
  ...
  async function loadOrders() {
    const rows = await listOrders({ search: qs.search, filters: qs.filters, sort: qs.sort, sortDir: qs.sortDir, sort2: qs.sort2, sortDir2: qs.sortDir2 });
  ...
  bindSortSelect(container.querySelector("[data-section='orders']"), qs, () => loadOrders().catch((e) => showToast(e.message, "error")));
  ...
  return { onSearch: (term) => { qs.search = term; loadOrders().catch((e) => showToast(e.message, "error")); } };
}
